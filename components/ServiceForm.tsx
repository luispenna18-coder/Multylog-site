...
const BRANDS = ["STILL", "Linde", "Toyota", "Hyster", "Yale", "Crown", "Baoli", "Mitsubishi", "Outra"];
...
export default function ServiceForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setTouched({ nome: true, email: true, empresa: true, telefone: true, marca: true });
    if (!form.nome || !form.email || !form.empresa || !form.marca || !isValidPhone(form.telefone)) return;
    ...
    return (
      <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-8 lg:p-10 flex flex-col items-center justify-center text-center gap-5 min-h-[420px]">
        ...
        <p className="text-gray-500 text-base max-w-sm leading-relaxed">
          Nossa equipe técnica vai analisar o seu chamado e entrar em contato em breve.
        </p>
        <button
  ...
  return (
    ...
    >
      ...
      <div>
        <label className={label}>Marca do equipamento <span className="text-[#CC0000]">*</span></label>
        <div className="relative">
          <select
            ...
            onBlur={handleBlur}
            required
            className={`${input} appearance-none cursor-pointer pr-10 ${touched.marca && !form.marca ? "border-[#CC0000]" : ""}`}
          >
